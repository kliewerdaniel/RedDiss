...
class LyricsGenerator:
    ...
    async def generate_lyrics(self, themes: Dict[str, Any]) -> Dict[str, Any]:
        """
        Generate diss track lyrics based on extracted themes
        ...
        context = self._build_context(themes)
    ...
    def _build_context(self, themes: Dict[str, Any]) -> str:
        """Build context for lyrics generation"""
        # Extract main themes
        main_themes = [t["theme"] for t in themes["main_themes"]]
        
        # Build prompt
        prompt = f"""Generate a diss track targeting {themes['target']} from r/{themes['context']}.
...
async def generate_lyrics(themes: Dict[str, Any]) -> Dict[str, Any]:
    """Main function to generate lyrics"""
    global _generator
    if _generator is None:
        _generator = LyricsGenerator()
    return await _generator.generate_lyrics(themes)
